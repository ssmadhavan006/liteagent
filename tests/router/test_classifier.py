...
def test_scorer_default_initialization():
    ...
    assert scorer.bias == -4.0
    assert scorer.weights["length_chars"] == 6.0
    assert scorer.theta_low == 0.35
...
def test_score_task_hand_computed():
    ...
    # dot_product = 0.0
    # raw_score = dot_product + bias = -4.0
    # score = 1.0 / (1.0 + exp(4.0))
    features = {
        "length_chars": 0.0,
    ...
    expected_score = 1.0 / (1.0 + math.exp(4.0))
